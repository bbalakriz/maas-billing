...
wait_for_pods() {
  ...
  oc get namespace "$namespace" &>/dev/null || return 0
  
  echo "⏳ Waiting for pods in $namespace to be ready..."
  local end=$((SECONDS + timeout))
  while [ $SECONDS -lt $end ]; do
    local not_ready=$(oc get pods -n "$namespace" --no-headers 2>/dev/null | grep -v -E 'Running|Completed|Succeeded' | wc -l)
...
wait_for_validating_webhooks() {
    ...
    while [ $SECONDS -lt $end ]; do
        ...
        local services
        services=$(oc get validatingwebhookconfigurations \
        ...
        for svc in $services; do
            ...
            ready=$(oc get endpoints -n "$ns" "$name" -o jsonpath='{.subsets[*].addresses[*].ip}' 2>/dev/null || true)
            if [ -z "$ready" ]; then
...
if ! oc get csv -n kuadrant-system kuadrant-operator.v1.3.0 &>/dev/null 2>&1; then
    echo "   No existing installation found, checking for leftover CRDs..."
    LEFTOVER_CRDS=$(oc get crd 2>/dev/null | grep -E "kuadrant|authorino|limitador" | awk '{print $1}')
...
cd "$PROJECT_ROOT"
oc apply --server-side=true --force-conflicts -f deployment/base/networking/odh/odh-gateway-api.yaml
oc apply --server-side=true --force-conflicts -f <(envsubst '$CLUSTER_DOMAIN' < deployment/base/networking/maas/maas-gateway-api.yaml)
...
cd "$PROJECT_ROOT"
oc apply -f deployment/base/networking/odh/kuadrant.yaml
...
oc rollout status deployment/kuadrant-operator-controller-manager -n kuadrant-system --timeout=60s || \
...
if [ -n "$AUD" ] && [ "$AUD" != "null" ]; then
    ...
    oc patch authpolicy maas-api-auth-policy -n maas-api \
      --type='json' \
      -p "$(jq -nc --arg aud "$AUD" '[{
        op:"replace",
...
oc -n kuadrant-system patch limitador limitador --type merge \
...
oc delete pod -n kuadrant-system -l control-plane=controller-manager 2>/dev/null && \
  echo "   ✅ Kuadrant operator restarted" || \
  echo "   ⚠️  Could not restart Kuadrant operator"

oc rollout restart deployment authorino-operator -n kuadrant-system 2>/dev/null && \
  echo "   ✅ Authorino operator restarted" || \
  echo "   ⚠️  Could not restart Authorino operator"

oc rollout restart deployment limitador-operator-controller-manager -n kuadrant-system 2>/dev/null && \
  echo "   ✅ Limitador operator restarted" || \
  echo "   ⚠️  Could not restart Limitador operator"

echo "   Waiting for operators to be ready..."
oc rollout status deployment kuadrant-operator-controller-manager -n kuadrant-system --timeout=60s 2>/dev/null || \
  echo "   ⚠️  Kuadrant operator taking longer than expected"
oc rollout status deployment authorino-operator -n kuadrant-system --timeout=60s 2>/dev/null || \
  echo "   ⚠️  Authorino operator taking longer than expected"
oc rollout status deployment limitador-operator-controller-manager -n kuadrant-system --timeout=60s 2>/dev/null || \
...
kustomize build deployment/base/observability | oc apply -f -
...
oc get pods -n maas-api --no-headers | grep Running | wc -l | xargs echo "  MaaS API pods running:"
oc get pods -n kuadrant-system --no-headers | grep Running | wc -l | xargs echo "  Kuadrant pods running:"
oc get pods -n opendatahub --no-headers | grep Running | wc -l | xargs echo "  KServe pods running:"

echo ""
echo "Gateway Status:"
oc get gateway -n openshift-ingress maas-default-gateway -o jsonpath='{.status.conditions[?(@.type=="Accepted")].status}' | xargs echo "  Accepted:"
oc get gateway -n openshift-ingress maas-default-gateway -o jsonpath='{.status.conditions[?(@.type=="Programmed")].status}' | xargs echo "  Programmed:"

echo ""
echo "Policy Status:"
oc get authpolicy -n openshift-ingress gateway-auth-policy -o jsonpath='{.status.conditions[?(@.type=="Accepted")].status}' 2>/dev/null | xargs echo "  AuthPolicy:"
oc get tokenratelimitpolicy -n openshift-ingress gateway-token-rate-limits -o jsonpath='{.status.conditions[?(@.type=="Accepted")].status}' 2>/dev/null | xargs echo "  TokenRateLimitPolicy:"



echo ""
echo "Policy Enforcement Status:"
oc get authpolicy -n openshift-ingress gateway-auth-policy -o jsonpath='{.status.conditions[?(@.type=="Enforced")].status}' 2>/dev/null | xargs echo "  AuthPolicy Enforced:"
oc get ratelimitpolicy -n openshift-ingress gateway-rate-limits -o jsonpath='{.status.conditions[?(@.type=="Enforced")].status}' 2>/dev/null | xargs echo "  RateLimitPolicy Enforced:"
oc get tokenratelimitpolicy -n openshift-ingress gateway-token-rate-limits -o jsonpath='{.status.conditions[?(@.type=="Enforced")].status}' 2>/dev/null | xargs echo "  TokenRateLimitPolicy Enforced:"
oc get telemetrypolicy -n openshift-ingress user-group -o jsonpath='{.status.conditions[?(@.type=="Enforced")].status}' 2>/dev/null | xargs echo "  TelemetryPolicy Enforced:"
...
echo "   oc describe authpolicy gateway-auth-policy -n openshift-ingress | grep -A 5 'Status:'"
echo ""
echo "2. If Gateway API provider is not installed, restart all Kuadrant operators:"
echo "   oc rollout restart deployment/kuadrant-operator-controller-manager -n kuadrant-system"
echo "   oc rollout restart deployment/authorino-operator -n kuadrant-system"
echo "   oc rollout restart deployment/limitador-operator-controller-manager -n kuadrant-system"
echo ""
echo "3. Check if OpenShift Gateway Controller is available:"
echo "   oc get gatewayclass"
echo ""
echo "4. If policies still show 'MissingDependency', ensure environment variable is set:"
echo "   oc get deployment kuadrant-operator-controller-manager -n kuadrant-system -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name==\"ISTIO_GATEWAY_CONTROLLER_NAMES\")]}'"
echo ""
echo "5. If environment variable is missing, patch the deployment:"
echo "   oc -n kuadrant-system patch deployment kuadrant-operator-controller-manager --type='json' \\"
...
echo "   oc rollout restart deployment/kuadrant-operator-controller-manager -n kuadrant-system"
echo "   oc rollout status deployment/kuadrant-operator-controller-manager -n kuadrant-system --timeout=60s"
echo ""
echo "7. Wait for policies to be enforced (may take 1-2 minutes):"
echo "   oc describe authpolicy gateway-auth-policy -n openshift-ingress | grep -A 10 'Status:'"
echo ""
echo "If metrics are not visible in Prometheus:"
echo "1. Check ServiceMonitor:"
echo "   oc get servicemonitor limitador-metrics -n kuadrant-system"
echo ""
echo "2. Check Prometheus targets:"
echo "   oc port-forward -n openshift-monitoring svc/prometheus-k8s 9090:9091 &"
...
echo "   oc rollout restart deployment/odh-model-controller -n opendatahub"
echo ""
echo "2. Temporarily bypass webhook:"
echo "   oc patch validatingwebhookconfigurations validating.odh-model-controller.opendatahub.io --type='json' -p='[{\"op\": \"replace\", \"path\": \"/webhooks/1/failurePolicy\", \"value\": \"Ignore\"}]'"
echo "   # Deploy your model, then restore:"
echo "   oc patch validatingwebhookconfigurations validating.odh-model-controller.opendatahub.io --type='json' -p='[{\"op\": \"replace\", \"path\": \"/webhooks/1/failurePolicy\", \"value\": \"Fail\"}]'"
echo ""
echo "If API calls return 404 errors (Gateway routing issues):"
echo "1. Check HTTPRoute status:"
echo "   oc get httproute -A"
echo "   oc describe httproute facebook-opt-125m-simulated-kserve-route -n llm"
echo ""
echo "2. Check if model is accessible directly:"
echo "   oc get pods -n llm"
echo "   oc port-forward -n llm svc/facebook-opt-125m-simulated-kserve-workload-svc 8080:8000 &"
echo "   curl -k https://localhost:8080/health"
echo ""
echo "3. Test model with correct name and HTTPS:"
echo "   curl -k -H \"Content-Type: application/json\" -d '{\"model\": \"facebook/opt-125m\", \"prompt\": \"Hello\", \"max_tokens\": 50}' https://localhost:8080/v1/completions"
echo ""
echo "4. Check Gateway status:"
echo "   oc get gateway -A"
echo "   oc describe gateway maas-default-gateway -n openshift-ingress"
echo ""
echo "If metrics are not generated despite successful API calls:"
echo "1. Verify policies are enforced:"
echo "   oc describe authpolicy gateway-auth-policy -n openshift-ingress | grep -A 5 'Enforced'"
echo "   oc describe ratelimitpolicy gateway-rate-limits -n openshift-ingress | grep -A 5 'Enforced'"
echo ""
echo "2. Check Limitador metrics directly:"
echo "   oc port-forward -n kuadrant-system svc/limitador-limitador 8080:8080 &"
...
echo "   for i in {1..5}; do curl -k -H \"Authorization: Bearer \$TOKEN\" -H \"Content-Type: application/json\" -d '{\"model\": \"facebook/opt-125m\", \"prompt\": \"Hello \$i\", \"max_tokens\": 50}' \"https://\${HOST}/llm/facebook-opt-125m-simulated/v1/completions\"; done"
...
echo "   kustomize build docs/samples/models/simulator | oc apply -f -"
echo ""
...
echo "   MODEL_URL=\"\${HOST}/llm/facebook-opt-125m-simulated/v1/completions\" # Note: This may be different for your model"
